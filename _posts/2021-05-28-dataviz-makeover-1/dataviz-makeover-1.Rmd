...
The original visualization are shown as below.

![](img/img01.png)

### 1.1 Clarity

1. The dual axis in each chart have different ranges. It would make the reader difficult to compare the amount of import and export.

2. The range of y axis in the six charts are different. Reader need extra effort to compare the trade amount of different countries. They have to look at the y coordinate scales separately in the six charts to get and compare the number.

3. The six charts have different widths. The different width will influence the reader how to interpret the change of the amount. If the width of the same x axis in the graph is longer, the data fluctuation will seem to be smaller.
...
1. The x-axis scale is confusing. It is not consistent with the axis title. The title is shown as 'Month', but the tick marker is year. Besides, the chart of the 'Japan' is not the same as the x-axis markings of other countries.

2. Font size and color are confusing. The gray color of the head's font is consistent with the black font of other labels. Besides, the font of the country is too large, even larger than the title.

3. The coordinates are repeated six times. They take up so much space in the picture that the area charts are very small.
...
The link to the original dashboard on Tableau Public Server can be found [here](https://public.tableau.com/app/profile/liu.yangguang/viz/A1_16222220286090/Dashboard2).
...
**No.** | **Step** | **Action**
-- | ------- | ------------------- 
1 | Import the data into Tableau and click "Cleaned with Data Interpreter" to get the cleaned table | ![](img/img02.png){width=100%}
...
6 | Hide "Variable" and "Variable - split 2" and rename the variables into meaningful name |![](img/img07.png){width=100%}
7 | Change data type of the "Date" variable from string to date in the two table  |![](img/img08.png){width=100%}
8 | Edit the connection between two tables to add one rule |![](img/img09.png){width=100%}
...
20 | Change the font and tick color to be consistent and publish the dashboard to Tableau Public |![](img/img21.png){width=100%}
