...
We can find that there are 44 employees, but 54 loyalty cards and 55 credit cards. If we ignore possible errors in the card number records, one employee has one or more credit cards and one or more loyalty cards.
...
card_correspond_true <- bind_rows(card_correspond_count_one2one, card_correspond_count_others_ture)
```

```{r}
ggplot(card_correspond_true,
       aes(y = 1, axis1 = last4ccnum, axis2 = loyaltynum)) +
  geom_alluvium(width = 1/10000) +
  # geom_stratum(width = 1/12, fill = "transparent", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("last4ccnum", "loyaltynum"), expand = c(.05, .05)) +
  scale_fill_brewer(type = "qual", palette = "Set1") + 
